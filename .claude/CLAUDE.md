...
- The admin session lives in its own store (`lib/adminAuth.ts`, `bookpool:admin-auth`) and admin calls pass its token via the `token` option, so admin login never writes to the user's `accessToken`. **A user already logged in with `role === 'ADMIN'` inherits the back office without a second login** — `useAdminAuth` derives an `Admin` from that user, and `getAdminToken()` returns `null` so the call rides the shared `accessToken` and keeps its `/reissue` retry (a copied token would be pinned and go stale). `linkedToUserSession` marks that state; `AdminLayout` offers "서비스로 나가기" instead of a logout there, because there is no separate admin session to end.
