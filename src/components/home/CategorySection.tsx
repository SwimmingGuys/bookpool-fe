...
const categories = [
  { icon: Code, label: 'IT/개발', count: 24, color: 'from-amber-700 to-amber-800' },
  { icon: Briefcase, label: '경영/경제', count: 18, color: 'from-orange-700 to-orange-800' },
  { icon: Palette, label: '예술/디자인', count: 12, color: 'from-stone-600 to-stone-700' },
  { icon: Heart, label: '자기계발', count: 31, color: 'from-amber-800 to-amber-900' },
  { icon: Globe, label: '인문/사회', count: 15, color: 'from-orange-800 to-orange-900' },
  { icon: GraduationCap, label: '학습/교육', count: 9, color: 'from-stone-700 to-stone-800' },
]
...
export default function CategorySection() {
  ...
  return (
    <Section
      ...
      blobs={[
        { className: 'top-0 right-0 w-96 h-96 bg-amber-100/15' },
        ...
        {categories.map((cat) => (
          <button
            ...
            className={cn(
              ...
              selected === cat.label
                ? 'border-amber-400 bg-white shadow-md scale-105'
